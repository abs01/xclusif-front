...
import { makeVip } from "../services/fetches";
...
export default function SinglePost() {
  ...
  useEffect(() => {
    if (post) {
      ...
      const currentUserId = JSON.parse(localStorage.getItem("account2"))?.user
        ?.id;

      setLikes(post.likes.some((like) => like.user_id === currentUserId));

      checkFollow(post.user_id).then((followData) => {
        const isFollowing = followData?.some((f) => f.id === currentUserId);
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-2xl mx-auto border-l border-r border-gray-800">
        ...
        <article className="border-b border-gray-800 p-4">
          ...
          <div className="flex gap-3 mb-4">
            ...
            {follow ? (
              <>
                <button className="flex items-center gap-2 hover:text-red-400 transition-colors group flex-1 justify-center py-2">
                  <RiUserUnfollowFill
                  ...
                  />
                  <span className="text-sm">Dejar de seguir</span>
                </button>
                <button className="flex items-center gap-2 hover:text-yellow-400 transition-colors group flex-1 justify-center py-2">
                  <RiVipCrown2Line
                    className="w-4 h-4 text-yellow-400"
                    onClick={(e) => {
                      if (
                        window.confirm(
                          "¿Estás COMPLETAMENTE seguro? Esta acción es irreversible y otorgará al usuario VIP, dándole acceso a funciones exclusivas y destacándolo en la plataforma.",
                        )
                      ) {
                        makeVip(e, post.user.id);
                      }
                    }}
                  />
          ...
          {/* Contenido del post */}
          <p className="text-xl leading-relaxed mb-4 break-words">
            {post.content}
          </p>
          {/* Imagen del post */}
          {post.media && (
          ...
          <div className="flex gap-6 py-4 border-t border-gray-800 text-gray-500 text-sm mb-4">
            <div className="hover:text-blue-400 cursor-pointer">
              <span className="font-bold text-white">{commentsLength}</span>{" "}
              comentarios
            </div>
            <div className="hover:text-pink-400 cursor-pointer">
              <span className="font-bold text-white">{likesLength}</span> me
              gusta
            </div>
          </div>
                ...
                <FaHeart
                  ...
                  onClick={() => {
                    ...
                    likePost(
                      false,
                      post.id,
                      post.likes[post.likes.length - 1]?.id,
                    );
        ...
        <div>
          {post.comments.map((comment, index) => {
            ...
            return (
              <article
                key={comment.id}
                className="flex gap-3 px-4 py-4 border-b border-gray-800 hover:bg-gray-900/40 transition-colors"
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-gray-100 text-sm leading-relaxed break-words mt-2">
                    {comment.content}
                  </p>
                </div>
      ...
      <Comment
        ...
        setCommentLength={setCommentsLength}
      />
